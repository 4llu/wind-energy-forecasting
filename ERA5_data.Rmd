...
```{r}
post_w10_w100_log <- stan_glmer(log_energy ~ w10 + w100 + (w10 + w100 | region) + (w10 + w100 | season) ,
                data = full_df,
                family = gaussian(link = "identity")
                )
```
...
#colnames(draws_w10_w100_log) <- c("a", "b0", "sigma")
...
```{r}
base <- ggplot(full_df, aes(x = w10, y = log_energy)) +
             geom_point(size=1)
base +
  ggtitle("w10 + w100 w/ region and log_energy 1") +
  geom_abline(data = draws_w10_w100_log, aes(intercept = a, slope = b0),
              color = "skyblue", size = 0.2, alpha = 0.25) +
  geom_abline(intercept = coef(post_w10_w100_log)[1], slope = coef(post_w10_w100_log)[2],
```    
